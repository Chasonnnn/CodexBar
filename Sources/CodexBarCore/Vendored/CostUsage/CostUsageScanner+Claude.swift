...
extension CostUsageScanner {
    ...
    /// Returns true when a JSONL file was generated by CodexBar's own internal background
    /// `claude /usage` probes and should be excluded from user-facing cost reporting.
    ///
    /// Claude CLI encodes the working-directory path into the project folder name by replacing
    /// path separators with hyphens (e.g. `/Library/Application Support/CodexBar/ClaudeProbe`
    /// → `-Library-Application-Support-CodexBar-ClaudeProbe`).
    /// We match against the two known encoded segments so the filter is
    /// OS-version and username independent.
    private static func isCodexBarInternalClaudeSession(_ url: URL) -> Bool {
        // The project directory is the grandparent of the JSONL file
        // (projects/<encoded-path>/<session>.jsonl  or
        //  projects/<encoded-path>/subagents/<session>.jsonl).
        let components = url.pathComponents
        // Walk upward to find the encoded project folder name.
        for component in components {
            ...
            if lower.contains("application-support-codexbar") {
                return true
            }
        }
        return false
        ...
        state: ClaudeScanState)
    {
        ...
        for case let url as URL in enumerator {
            ...
            // Skip JSONL files generated by CodexBar's own internal Claude probe sessions.
            // Claude CLI encodes the working directory as the project folder name using path
            // components joined by hyphens. We match known CodexBar-internal path segments
            // so that background /usage probe sessions never appear as user usage.
            guard !Self.isCodexBarInternalClaudeSession(url) else { continue }
