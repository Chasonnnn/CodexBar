...
enum CostUsagePricing {
    ...
    /// OpenCode aliases normalize into these canonical keys before lookup.
    ...
    static func normalizeCodexModel(_ raw: String) -> String {
        ...
        var strippedOpenCodeSuffix = false
        for suffix in ["-xhigh", "-high", "-medium", "-low", "-thinking"] where trimmed.hasSuffix(suffix) {
        ...
        if self.codex[trimmed] != nil {
            if strippedOpenCodeSuffix,
               let codexRange = trimmed.range(of: "-codex")
            {
    ...
    static func normalizeClaudeModel(_ raw: String) -> String {
        ...
        for suffix in ["-thinking", "-preview"] where trimmed.hasSuffix(suffix) {
            let candidate = String(trimmed.dropLast(suffix.count))
